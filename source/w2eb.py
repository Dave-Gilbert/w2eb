...
def sketch_heading(heading):
    """
    @summary: 
    
    Convert a title like '1.0 Important Dates' to a normalized format
...
def sketch_paragraph(paragraph):
    """
    Take the first 8 words in a paragraph and normalize them by
...
def sketch_page(bl):    
    ...
    for tag in bl.find_all('h1') + bl.find_all('h2') + bl.find_all('h3'):
        sktxt = sketch_heading(get_text_safe(tag))
        if sktxt:
            sketch = sketch.union({sktxt})

    for tag in bl.find_all('p'):
        sktxt = sketch_paragraph(get_text_safe(tag))
        if sktxt:
            sketch = sketch.union({sktxt})

    return sketch

def parent_sketch_vs_my_sketch(parent_sketch, my_sketch):
...
def AddSectionsToMain(opts, bl, sect_label_href_list):
    ...
    for section in sect_label_href_list:
        ...
        main_sketch = sketch_page(bl)
        sect_sketch = sketch_page(sl)
        similar = parent_sketch_vs_my_sketch(main_sketch, sect_sketch)
        if similar:
...
def AddSummariesToMain(opts, bl, foot_dict_list):
...
def AddDebugHeadings(opts, bl):

    #
    # Must add debug headings prior to entries here so they appear in TOC
    #
    if opts['debug'] == 0:
        return
...
def MainSectionCreate(opts, st_time, bl, section_bname):
    ...
    if opts['parent'] == '':
        final_sect_label_href_list = AddSectionsToMain(opts, bl, sect_label_href_list)
        AddSummariesToMain(opts, bl, foot_dict_list)
        while TocRemoveOldToc(opts, bl) == '':
            old_toc += 1
        AddDebugHeadings(opts, bl)
...
def main(opts):
    ...
    if not opts['footnote']:
        opts['my_sketch'] = sketch_page(bl)
        i_am_my_parent = parent_sketch_vs_my_sketch(opts['parent_sketch'], 
                                                    opts['my_sketch'])
